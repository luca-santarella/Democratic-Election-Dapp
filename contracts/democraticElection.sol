...
import "/contracts/SOULToken.sol";
...
contract democraticElection {
    ...
    function open_envelope(uint _sigil, address _symbol, uint _souls) canOpen public{
        ...
        preferenceCandidates[_symbol].soul = _souls;
    ...
    function mayor_or_sayonara() canCheckOutcome public {
        ...
        if(!isTie)
            ...
            winningCandidate = payable(bestCandidate);
        
        //Refunding losing voters
        for(uint i=0; i < voters.length; i++){
            address payable voterAddress = payable(voters[i]);
            if(souls[voterAddress].symbol != winningCandidate){
                uint refundedSouls = souls[voterAddress].soul;
                souls[voterAddress].soul = 0;
                token.transferFrom(address(this), voterAddress, refundedSouls);
            }
            else{
                token.transferFrom(address(this), winningCandidate, souls[voterAddress].soul);
            }
        }
        
        if(isTie)
            emit NewMayor(winningCandidate);
        else
            emit Tie();
    }
